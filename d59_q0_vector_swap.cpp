...
void vector_swap(vector<int> &v1, vector<int> &v2, int start1, int end1, int start2, int end2) {
	...
	for(int i = end1 - 1; i >= start1; i--)
		a.push_back(v1[i]), v1.erase(v1.begin() + i);
	for(int i = end2 - 1; i >= start2; i--)
		b.push_back(v2[i]), v2.erase(v2.begin() + i);
	v1.insert(v1.begin() + start1, b.rbegin(), b.rend());
	v2.insert(v2.begin() + start2, a.rbegin(), a.rend());
}
int main() {
	// read input
	int n, c;
